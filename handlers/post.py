...
# photo sent with no active state -> start the flow directly
...
async def got_caption(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        await message.answer("Failed to process the image. Please try again by sending a photo.")
        await state.clear()
        return
...
# a new photo sent WHILE waiting for a caption -> treat it as restarting with the new photo
@router.message(PostStates.waiting_for_caption, F.photo)
async def photo_while_waiting_caption(message: Message, state: FSMContext):
    await state.update_data(photo_file_id=message.photo[-1].file_id)
    await message.answer("Got a new photo. Now send the caption.")
